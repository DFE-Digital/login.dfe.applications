...
const { find, findAll, unusedServiceFields } = require("./data");
...
const buildWhere = (ids, clientIds) => {
  ...
  if (ids.length > 0 && clientIds.length > 0) {
    where = {
      [Op.or]: [{ id: { [Op.in]: ids } }, { clientId: { [Op.in]: clientIds } }],
    };
  } else if (ids.length > 0) {
    where =
      ids.length > 1 ? { id: { [Op.in]: ids } } : { id: { [Op.eq]: ids[0] } };
  } else {
    where =
      clientIds.length > 1
        ? { clientId: { [Op.in]: clientIds } }
        : { clientId: { [Op.eq]: clientIds[0] } };
  }
...
const getSummaries = async (req, res) => {
  const serviceIds = req.params.ids
  ...
  serviceIds.forEach((id) =>
    isUUID(id) ? queryIds.push(id) : queryClientIds.push(id),
  );

  const fields = req.query.fields
    ? [...new Set(req.query.fields.split(","))]
    : [];
  // The below attributes/associations are not used in the returned object.
  const serviceAttributes = Object.keys(
    dataModel.services.rawAttributes,
  ).filter((field) => !unusedServiceFields.includes(field));
  const serviceAssociations = Object.keys(
    dataModel.services.associations,
  ).filter((field) => !unusedServiceFields.includes(field));
  ...
  fields.forEach((field) =>
    serviceAttributes.includes(field)
      ? queryAttributes.push(field)
  ...
  const queryOptions = {
    ...
    attributes:
      queryAttributes.length > 0 || fields.length > 0
        ? queryAttributes
        : serviceAttributes,
    include:
      queryAssociations.length > 0 || fields.length > 0
        ? queryAssociations
        : serviceAssociations,
  };
  ...
  try {
    const serviceInfo =
      serviceIds.length > 1
        ? await getMultipleServices(queryOptions)

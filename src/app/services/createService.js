...
const parseAndValidateRequest = async (req) => {
    ...
    service: {
      ...
      relyingParty: {
        ...
        token_endpoint_auth_method:
          relyingParty.token_endpoint_auth_method || undefined,
  ...
  if (!model.service.relyingParty.client_secret) {
    ...
    model.validationErrors.push(
      "relyingParty.client_secret must be at least 15 characters",
    );
  } else {
    ...
    if (existingServiceWithClientId) {
      model.validationErrors.push(
        `relyingParty.client_id must be unique. ${model.service.relyingParty.client_id} is already assigned`,
      );
    }
  }
...
const createService = async (req, res) => {
  ...
  await forEachAsync(
    model.service.relyingParty.redirect_uris,
    async (redirectUri) => {
      await addRedirectUri(id, redirectUri);
    },
  );
  await forEachAsync(
    model.service.relyingParty.post_logout_redirect_uris,
    async (redirectUri) => {
      await addPostLogoutRedirect(id, redirectUri);
    },
  );
  await forEachAsync(
    model.service.relyingParty.grant_types,
    async (grantType) => {
      await addGrantType(id, grantType);
    },
  );
  await forEachAsync(
    model.service.relyingParty.response_types,
    async (responseType) => {
      await addResponseType(id, responseType);
    },
  );
  await forEachAsync(
    Object.keys(model.service.relyingParty.params),
    async (paramName) => {

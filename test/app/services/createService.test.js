...
const service = {
  ...
  relyingParty: {
    ...
    redirect_uris: [
      "https://unit.test/auth/callback",
      "https://unit.test2/auth/callback",
    ],
    post_logout_redirect_uris: [
      "https://unit.test/signout/callback",
      "https://unit.test2/signout/callback",
    ],
...
describe("when creating a service", () => {
  ...
  it("then it should create all redirect uris", async () => {
    ...
    expect(addRedirectUri).toHaveBeenCalledWith(
      id,
      service.relyingParty.redirect_uris[0],
    );
    expect(addRedirectUri).toHaveBeenCalledWith(
      id,
      service.relyingParty.redirect_uris[1],
    );
  });
  ...
  it("then it should create all post logout redirect uris", async () => {
    ...
    expect(addPostLogoutRedirect).toHaveBeenCalledWith(
      id,
      service.relyingParty.post_logout_redirect_uris[0],
    );
    expect(addPostLogoutRedirect).toHaveBeenCalledWith(
      id,
      service.relyingParty.post_logout_redirect_uris[1],
    );
  });
  ...
  it("then it should create all grant types", async () => {
    ...
    expect(addGrantType).toHaveBeenCalledWith(
      id,
      service.relyingParty.grant_types[0],
    );
    expect(addGrantType).toHaveBeenCalledWith(
      id,
      service.relyingParty.grant_types[1],
    );
    expect(addGrantType).toHaveBeenCalledWith(
      id,
      service.relyingParty.grant_types[2],
    );
  });
  ...
  it("then it should create all response types", async () => {
    ...
    expect(addResponseType).toHaveBeenCalledWith(
      id,
      service.relyingParty.response_types[0],
    );
    expect(addResponseType).toHaveBeenCalledWith(
      id,
      service.relyingParty.response_types[1],
    );
  });
  ...
  it("then it should create all service params", async () => {
    ...
    expect(addServiceParam).toHaveBeenCalledWith(
      id,
      "explicitConsent",
      service.relyingParty.params.explicitConsent,
    );
  });
  ...
  it("then it should return bad request if request missing client id", async () => {
    req.body = Object.assign({}, service, {
      relyingParty: Object.assign({}, service.relyingParty, {
        client_id: undefined,
      }),
    });
  ...
  it("then it should return bad request if client id is uuid", async () => {
    req.body = Object.assign({}, service, {
      relyingParty: Object.assign({}, service.relyingParty, {
        client_id: "020840b2-8e78-4e8c-a09c-e38e8cdd78d1",
      }),
    });
  ...
  it("then it should return bad request if client id is already in use", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      reasons: [
        "relyingParty.client_id must be unique. existing-client-id is already assigned",
      ],
    });
  });
